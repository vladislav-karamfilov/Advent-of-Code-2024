...
fn calculate_min_steps_to_end(
    ...
    while let Some((current_state, _)) = states.pop() {
        ...
        if !visited.insert((current_state.coordinate, current_state.steps)) {
            continue;
        }

        let next_states = calculate_next_states(&current_state, end, memory_space_grid);

        for next_state in next_states {
            if visited.contains(&(next_state.coordinate, next_state.steps)) {
                continue;
            }
...
fn calculate_next_states(
    ...
    if coordinate.y > 0 && memory_space_grid[coordinate.y - 1][coordinate.x] == '.' {
    ...
    if coordinate.y < memory_space_grid.len() - 1
        && memory_space_grid[coordinate.y + 1][coordinate.x] == '.'
    {
        result.push(PathState {
            coordinate: Coordinate2D {
                y: coordinate.y + 1,
    ...
    if coordinate.x > 0 && memory_space_grid[coordinate.y][coordinate.x - 1] == '.' {
        result.push(PathState {
            coordinate: Coordinate2D {
                x: coordinate.x - 1,
                ..coordinate
    ...
    if coordinate.x < memory_space_grid.len() - 1
        && memory_space_grid[coordinate.y][coordinate.x + 1] == '.'
    {
        result.push(PathState {
            coordinate: Coordinate2D {
                x: coordinate.x + 1,
